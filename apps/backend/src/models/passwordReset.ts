...
export class PasswordResetRepository {
  ...
  static async findActiveByUserId(userId: string): Promise<PasswordReset[]> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      "SELECT * FROM password_resets WHERE user_id = ? AND used = 0 AND expires_at > UTC_TIMESTAMP()",
      [userId],
    );
  ...
  static async findAndVerify(plainToken: string): Promise<PasswordReset | null> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      "SELECT * FROM password_resets WHERE used = 0 AND expires_at > UTC_TIMESTAMP()",
    );
